...
def callback(msg):
   global orig_min,region1,region2,region3
...
def wall():
   global orig_min,region1,region2,region3,min1,min2,min3
   x= orig_min+0.5
   while not rospy.is_shutdown():
       min1=min(region1)
       min2=min(region2)
       min3=min(region3)
       if (x-0.3<=min1<=x+0.3 and min2>x):
          teleopbot(50,-50,50,-50)
       elif (min1>x+0.3 and min2>x) :
        teleopbot(50,50,50,50)
       elif (min1<x-0.3 and min2<x):
         teleopbot(-50,-50,-50,-50)
       #elif(0.24<min2<0.35):
